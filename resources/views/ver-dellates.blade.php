{{-- resources/views/ver-dellates.blade.php --}}
<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>{{ $pension->nombre }} - UniRooms (Propietario)</title>
...
@php
    ...
    // link_foto puede venir casteado como array (cast en el modelo) o como JSON texto
    ...
    $estado   = optional($pension->estado)->nombre ?? 'Sin estado';
    $disponibleTexto = $pension->disponible ? 'Disponible' : 'No disponible';
    $disponibleColor = $pension->disponible ? 'text-green-600 bg-green-50' : 'text-red-600 bg-red-50';
...
<body class="bg-gray-100 min-h-screen flex flex-col">
  ...
  <main class="flex-1">
    <div id="app">
      <div class="min-h-screen bg-[var(--background)]">
        ...
        {{-- HEADER interno de detalle (botón Volver y título) --}}
          ...
          <div class="mt-6 grid gap-5 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
            ...
            <section>
              <div class="bg-white rounded-2xl shadow-md p-5 space-y-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                  <div>
                    <div class="flex items-center gap-2 mb-1">
                      <h2 class="text-xl font-semibold">{{ $pension->nombre }}</h2>
                      <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] {{ $disponibleColor }}">
                ...
                {{-- Descripción --}}
                <div>
                  <h3 class="text-base font-semibold mb-1.5">Descripción para estudiantes</h3>
                ...
                <div>
                  ...
                  @if($pension->servicios->isEmpty())
                    <p class="text-sm text-[var(--muted-foreground)]">
                      No has asociado servicios a esta habitación.
                    </p>
                  @else
                            ...
                            @switch($servicio->id)
                              ...
                              @case(5) {{-- Cocina --}}
                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                     viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                     stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                     data-lucide="chef-hat" class="lucide lucide-chef-hat">
                              ...
                              @case(6) {{-- Baño privado --}}
                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                     viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                     stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                     data-lucide="bath" class="lucide lucide-bath">
                              ...
                              @case(7) {{-- Lavandería --}}
                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                     viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                     stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                     data-lucide="shirt" class="lucide lucide-shirt">
                              ...
                              @case(8) {{-- Parqueadero --}}
                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                     viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                     stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                     data-lucide="car" class="lucide lucide-car">
                              ...
                              @case(10) {{-- Jardín --}}
                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                  ...
                                  <path d="M12 2v8"></path>
                                  <path d="M2 12h20"></path>
                                  <path d="m20 10-2-2-4 4-2-2-4 4-2-2-4 4"></path>
                                </svg>
                                @break
                              @case(11) {{-- Gimnasio --}}
                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                     viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                     stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                     data-lucide="dumbbell" class="lucide lucide-dumbbell">
                              ...
                              @case(12) {{-- Sala de estudio --}}
                                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                     viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                     stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                     data-lucide="book" class="lucide lucide-book">
            ...
            <aside class="flex flex-col gap-4">
              ...
              <div class="bg-white rounded-2xl shadow-md p-4 space-y-3">
                <h3 class="text-base font-semibold">Estado de la publicación</h3>
                ...
                <div class="space-y-1 text-sm">
                  ...
                  <div class="flex justify-between">
                    ...
                    <span class="font-semibold">{{ $disponibleTexto }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span>Precio mensual:</span>
                    <span class="font-semibold">$ {{ number_format($precio, 0, ',', '.') }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span>Creada el:</span>
                    <span class="text-xs">
                      {{ optional($pension->created_at)->format('d/m/Y') ?? 'N/D' }}
                    </span>
                  </div>
                  <div class="flex justify-between">
                    <span>Última actualización:</span>
                    <span class="text-xs">
                      {{ optional($pension->updated_at)->format('d/m/Y H:i') ?? 'N/D' }}
                    </span>
                  </div>
                </div>

                <p class="text-xs text-[var(--muted-foreground)] mt-2">
                  Esta vista te permite revisar cómo se muestra tu habitación a los estudiantes
                  y acceder rápidamente a las reservas relacionadas.
                </p>
              </div>

              {{-- Gestión rápida --}}
              <div class="bg-white rounded-2xl shadow-md p-4 space-y-3">
                <h3 class="text-base font-semibold">Gestión rápida</h3>

                 <a href="{{ route('pension.edit', $pension) }}"
                    class="btn btn-primary w-full rounded-full inline-flex items-center justify-center gap-2 text-sm mb-2">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                        viewBox="0 0 24 24" fill="none" stroke="currentColor"
                        stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                        data-lucide="pencil" class="lucide lucide-pencil">
                ...
                <a href="{{ route('propietario.solicitudes') }}"
                ...
                <button id="share-room" type="button"
                  ...
                  Compartir enlace de la habitación
                </button>

                <p class="text-xs text-[var(--muted-foreground)] text-center">
                  Puedes copiar y compartir el enlace de esta habitación con posibles inquilinos.
                </p>
              </div>
            </aside>
          </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // Volver a listado de habitaciones del propietario
      const backBtn = document.getElementById('back-to-list');
      backBtn?.addEventListener('click', () => {
        if (document.referrer) {
          window.history.back();
        } else {
          window.location.href = "{{ route('propietario.habitaciones') }}";
        }
      });

      // Compartir enlace (API nativa si está disponible)
      const shareBtn = document.getElementById('share-room');
      shareBtn?.addEventListener('click', async () => {
        const shareData = {
          title: "{{ $pension->nombre }} - UniRooms",
          text: "Mira esta habitación publicada en UniRooms:",
